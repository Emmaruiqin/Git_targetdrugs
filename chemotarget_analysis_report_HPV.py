...
def add_basic_informmationHPV(doc, informdict, barcode):
    nowtime = time.strftime('%Y/%m/%d', time.localtime())
    doctable = doc.Tables[1]
    doctable1 = doc.Tables[4]
    doctable.Cell(1, 1).Range.Text = "条形码:" + str(barcode)
    doctable.Cell(1, 2).Range.Text = "样本号:" + informdict[barcode]['样本号']
    doctable.Cell(2, 2).Range.Text = informdict[barcode]['姓名']
    doctable.Cell(2, 4).Range.Text = str(informdict[barcode]['岁'])
    doctable.Cell(2, 6).Range.Text = informdict[barcode]['临床诊断']
    doctable.Cell(4, 2).Range.Text = informdict[barcode]['医院名称']
    doctable.Cell(4, 4).Range.Text = informdict[barcode]['科室']
    doctable.Cell(4, 6).Range.Text = informdict[barcode]['送检医生']
    doctable.Cell(5, 2).Range.Text = informdict[barcode]['病人号']
    if informdict[barcode]['采集时间'] != '':
        doctable.Cell(5, 4).Range.Text = informdict[barcode]['采集时间'].strftime('%Y/%m/%d')
    else:
        doctable.Cell(5, 4).Range.Text = ''
    doctable.Cell(5, 6).Range.Text = informdict[barcode]['录入时间'].strftime('%Y/%m/%d')
    doctable1.Cell(1, 4).Range.Text = informdict[barcode]['检测时间'].strftime('%Y/%m/%d')
    doctable1.Cell(2, 4).Range.Text = nowtime
    return doc

def add_HPVresult(doc, barcode,filename):
    workbook = openpyxl.load_workbook(r'%s' % (filename), data_only=True)
    hpv=[]
    for i in range(2,25):
        name = workbook[barcode].cell(row=i, column=1).value
        hpv.append(str(name))
    result = []
    change =[]
    for i in range(2,25):
        result1 = workbook[barcode].cell(row=i, column=2).value
        result.append(str(result1))
    doctable = doc.Tables[2]
    doctable1 = doc.Tables[3]
    for i in range(2,14):
        doctable.Cell(i, 1).Range.Text = hpv[i - 2]
        doctable.Cell(i, 2).Range.Text = result[i - 2]
        if result[i-2]=="阳性":
            #table.Cell(i, 2).paragraphs[0].style.font.color.rgb = RGBColor(230, 0, 0)
            change.append(str(hpv[i - 2])[3:])
    for i in range(2, 13):
        doctable.Cell(i, 4).Range.Text = hpv[i +10]
        doctable.Cell(i, 5).Range.Text = result[i +10]
        if result[i+10]=="阳性":
            #table.Cell(i, 5).paragraphs[0].style.font.color.rgb = RGBColor(230, 0, 0)
            change.append(str(hpv[i + 10])[3:])
    xianshi=""
    for i in range(0,len(change)):
        xianshi=xianshi+change[i]+"，"
    if len(change)>1:
        doctable1.Cell(1, 1).Range.Text="该检测样本为HPV"+xianshi[:-1]+"型多基因混合感染，其临床意义建议医生结合具体病情进行综合分析。"
    else:
        doctable1.Cell(1, 1).Range.Text = "该检测样本为HPV" + xianshi[:-1] + "型单基因感染，其临床意义建议医生结合具体病情进行综合分析。"
    return doc
...
def main(Expresultfiles):
    ...
    for file in Expresultfiles:
        ...
        for eachsample in informdict.keys():  # 打印正在生成的检测者
            ...
            if 'HPV' in informdict[eachsample]['检验目的名称']:
                reporttemplate_3 = u'D:\\HPV报告模板.doc'
                reportname = os.path.join(os.getcwd(), '%s_%s_%s.doc' % (eachsample, informdict[eachsample]['姓名'], informdict[eachsample]['医院名称']))
                reportname2 = os.path.join(os.getcwd(), '%s_%s_%s.pdf' % (eachsample, informdict[eachsample]['姓名'], informdict[eachsample]['医院名称']))
                copyfile(reporttemplate_3, reportname)

                w = win32com.client.Dispatch('Word.Application')
                w.Visible = 0
                w.DisplayAlerts = 0
                doc = w.Documents.Open(FileName=reportname)
                doc = add_basic_informmationHPV(doc=doc, informdict=informdict, barcode=eachsample)  # 添加每个受试者的个人信息
                doc = add_HPVresult(doc=doc,barcode=eachsample,filename=file)
                doc = HPVcolor(doc=doc)
                ...
                subtabnum = 0
                for tabnum in range(bgtable_num, doc.Tables.Count):
